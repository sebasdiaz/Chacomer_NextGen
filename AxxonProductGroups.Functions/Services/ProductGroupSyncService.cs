...
namespace AxxonProductGroups.Functions.Services
{
    ...
    ///   dataAreaId -> owningbusinessunit  (lookup businessunit por name)
    ///
    ...
    ///   2. Resuelve owningbusinessunit via businessunit.name = dataAreaId.
    ///      Si no se encuentra, el registro se upsertea sin owningbusinessunit
    ///      y se loguea Warning.
    ...
    public class ProductGroupSyncService : IProductGroupSyncService
    {
        ...
        private readonly Dictionary<string, Guid?> _buCache      = new(StringComparer.OrdinalIgnoreCase);
        ...
        private Entity? MapToEntity(FoProductGroup g)
        {
            ...
            var buId = ResolveBusinessUnit(g.DataAreaId);
            if (buId.HasValue)
                e["owningbusinessunit"] = new EntityReference("businessunit", buId.Value);
            else
                _logger.LogWarning(
                    "[ProductGroupSyncService] BusinessUnit no encontrada para DataAreaId={DataAreaId} " +
                    "(GroupId={GroupId}). El registro quedara sin owningbusinessunit.",
                    g.DataAreaId, g.GroupId);
        ...
        private Guid? ResolveBusinessUnit(string dataAreaId)
        {
            if (string.IsNullOrEmpty(dataAreaId))
                return null;

            if (_buCache.TryGetValue(dataAreaId, out var cached))
                return cached;
            ...
            query.Criteria.AddCondition("name", ConditionOperator.Equal, dataAreaId);

            var result = _orgService.RetrieveMultiple(query);
            if (result.Entities.Count == 0)
            {
                _buCache[dataAreaId] = null;
                return null;
            }

            var id = result.Entities[0].Id;
            _buCache[dataAreaId] = id;
            return id;
        }
    }
}
