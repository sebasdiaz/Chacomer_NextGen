...
> **Estado: la app existe en INTE, sin código todavía.** `fa-axxoncustomercredit-inte`
> está creada y con sus roles; falta el alta de su Managed Identity en F&O y la definición
> del pipeline en ADO. Probado end-to-end desde una máquina de desarrollo contra F&O INTE.
...
durante días. Éste es el estado real de INTE al 2026-09-02:

1. ✅ **Función App creada.** El pipeline de infra corrió con `deployCustomerCreditApp = true`
   en `inte.bicepparam` y creó `fa-axxoncustomercredit-inte` y su storage
   (`stcustcreditinte5uzv3dgp`).
2. ✅ **Roles de Storage y Key Vault asignados a mano** a la MI
...
3. ⬜ **Dar de alta esa MI en F&O INTE**, con lectura sobre las cuatro entidades
   `DevAxCustCredit*`. Sin eso los endpoints responden `502` sin más pistas.
4. ⬜ **Dar de alta la definición del pipeline en Azure DevOps** y autorizarle la service
   connection y el environment. El YAML en el repo no crea la definición — ver
   [Doble PR](../runbooks/doble-pr.md). Con eso corrido, `deployToInte: true` (ya está en el
   YAML) despliega el código.
